...
def main():
    ...
    parser.add_argument(
        ...
        default="DATA",
        type=str,
        help="The names of the data columns in the MS file to image. E.g. 'AST_DATA,TAB_DATA,CAL_DATA'.",
    )
    parser.add_argument(
        "-n",
        "--name_suffix",
    ...
    parser.add_argument(
        "-w",
        "--wsclean_opts",
        default="-size 2048 2048 -scale 2asec -niter 50000 -mgain 0.3 -auto-threshold 1 -pol xx -weight briggs -0.5 -auto-mask 3",
        help="WSclean options. default is '-size 2048 2048 -scale 2asec -niter 50000 -mgain 0.3 -auto-threshold 1 -pol xx -weight briggs -0.5 -auto-mask 3'",
    )
    parser.add_argument(
        "-b",
        "--bash_exec",
    ...
    container_cmd = f"docker run {docker_opts} -v {data_dir}:/data --workdir /data/images chrisjfinlay/wsclean:3.4"

    container_cmd = f"podman run --userns=host --privileged -v {data_dir}:/data --workdir /data/images docker.io/chrisjfinlay/wsclean:3.4"

    if sif_path is not None:
        sif_path = os.path.abspath(sif_path)
        container_cmd = (
            f"singularity exec --bind {data_dir}:/data --pwd /data/images {sif_path}"
        )
